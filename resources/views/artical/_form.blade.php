<form  method="post" action="" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        value="{{ old('source') ? old('source') : $artical->picture }}" onchange="PreviewImage(this)">

        <div id="imgPreview" style=" margin:10px 0 0 315px; ">
            <img style="height: 100px;width: 120px" @if($artical->picture!='') src="{{ asset('upload/'.$artical->picture) }}" @else display="none" @endif alt="文章封面预览">
        </div>
    </div>

    <div id="groups" class="form-group">
        <label>Text area</label>
        <textarea id="editor" class="form-control" style="padding:0;" name="Artical[articalcontent]"  >{{ old('Artical')['articalcontent'] ? old('Artical')['articalcontent'] : $artical->articalcontent }}</textarea>
    </div>
